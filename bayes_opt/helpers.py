...
def acq_local_max(ac, gp, y_max, points, bounds, random_state, dist=0.05, n_warmup=2000, n_iter=5):

    boxes = []
    dists = dist*(bounds[:, 1]-bounds[:,0]) + bounds[:,0]
    for pt in points:
        boxes.append(np.column_stack((np.maximum(bounds[:, 0], pt - dists),
                                      np.minimum(bounds[:, 1], pt + dists))))

    max_acq = - np.inf
    x_max = None
    for box in boxes:
        x_tries = random_state.uniform(box[:, 0], box[:, 1],
                                       size=(n_warmup, bounds.shape[0]))
        ...
        if found > max_acq:
            max_acq = found
            x_max = np.clip(x_tries[ys.argmax()], box[:, 0], box[:, 1])

    # Explore the parameter space more throughly
    for box in boxes:
        x_seeds = random_state.uniform(box[:, 0], box[:, 1],
                                       size=(n_iter, bounds.shape[0]))
        for x_try in x_seeds:
            # Find the minimum of minus the acquisition function
            res = minimize(lambda x: -ac(x.reshape(1, -1), gp=gp, y_max=y_max),
            ...
            # Store it if better than previous minimum(maximum).
            if -res.fun[0] > max_acq:
                x_max = np.clip(res.x, box[:, 0], box[:, 1])
                max_acq = -res.fun[0]
